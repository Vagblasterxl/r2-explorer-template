...
1a. Log the fire:
    ...
    ('cannon.fired', 'fortress', json_object('timestamp', datetime('now')));

1b. Pause check:
    SELECT value FROM cannon_state WHERE key='fortress_paused';
...
1c. Zombie reaper (stuck >15min):
    ...
    WHERE instance_target='fortress' AND status='running'
...
=====================================================================
    WITH selected AS (
      ...
      WHERE instance_target='fortress' AND status='waiting'
        AND (scheduled_for IS NULL OR scheduled_for <= datetime('now'))
      ORDER BY priority ASC, id ASC
      LIMIT 4
    )
    UPDATE task_pool
    SET status='running', claimed_at=datetime('now'), claimed_by='fortress-cannon'
...
For each task in the salvo:
  ...
  ON SUCCESS:
    ...
    ('cannon.completed', 'fortress', ?, json_object('task_name', ?, 'category', ?, 'duration_seconds', ?));
  ...
  ON FAILURE (genuinely blocked, not just hard):
    ...
    WHERE id=?;
    INSERT INTO events (event_type, source, task_id, payload) VALUES
    ('cannon.task_failed', 'fortress', ?, json_object('error', ?, 'will_retry', ?));
...
=====================================================================
    INSERT INTO events (event_type, source, payload) VALUES
    ('cannon.salvo_complete', 'fortress', json_object(
...
If PHASE 2 returned nothing:
    INSERT INTO events (event_type, source, payload) VALUES
    ('cannon.pool_empty', 'fortress', json_object('message', 'FORTRESS pool drained'));
...
Runaway guard:
    SELECT value FROM cannon_state WHERE key='fortress_chain_depth_today';
    ...
    Else: UPDATE cannon_state SET value=CAST(value AS INTEGER)+1, updated_at=datetime('now')
          WHERE key='fortress_chain_depth_today';

Then call create_scheduled_task with:
  - taskId: 'task-cannon-fortress-' + Date.now()   (UNIQUE — never collides)
...
- Focus is security and identity hardening for a solo sole-admin operator until cannon_state says otherwise. No drift.
```
