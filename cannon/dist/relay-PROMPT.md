...
1a. Log the fire:
    ...
    ('cannon.fired', 'relay', json_object('timestamp', datetime('now')));

1b. Pause check:
    SELECT value FROM cannon_state WHERE key='relay_paused';
...
1c. Zombie reaper (stuck >15min):
    ...
    WHERE instance_target='relay' AND status='running'
...
=====================================================================
    WITH selected AS (
      ...
      WHERE instance_target='relay' AND status='waiting'
        AND (scheduled_for IS NULL OR scheduled_for <= datetime('now'))
      ORDER BY priority ASC, id ASC
      LIMIT 4
    )
    UPDATE task_pool
    SET status='running', claimed_at=datetime('now'), claimed_by='relay-cannon'
...
For each task in the salvo:
  ...
  ON SUCCESS:
    ...
    ('cannon.completed', 'relay', ?, json_object('task_name', ?, 'category', ?, 'duration_seconds', ?));
  ...
  ON FAILURE (genuinely blocked, not just hard):
    ...
    WHERE id=?;
    INSERT INTO events (event_type, source, task_id, payload) VALUES
    ('cannon.task_failed', 'relay', ?, json_object('error', ?, 'will_retry', ?));
...
=====================================================================
    INSERT INTO events (event_type, source, payload) VALUES
    ('cannon.salvo_complete', 'relay', json_object(
...
If PHASE 2 returned nothing:
    INSERT INTO events (event_type, source, payload) VALUES
    ('cannon.pool_empty', 'relay', json_object('message', 'RELAY pool drained'));
...
Runaway guard:
    SELECT value FROM cannon_state WHERE key='relay_chain_depth_today';
    ...
    Else: UPDATE cannon_state SET value=CAST(value AS INTEGER)+1, updated_at=datetime('now')
          WHERE key='relay_chain_depth_today';

Then call create_scheduled_task with:
  - taskId: 'task-cannon-relay-' + Date.now()   (UNIQUE — never collides)
...
- Focus is Slack internal bot and cross-service comms bridge (Slack, Google Workspace, cannon control) until cannon_state says otherwise. No drift.
```
